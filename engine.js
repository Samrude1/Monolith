...
export class GameEngine {
    ...
    updateEntities(dt) {
        ...
        for (const ent of this.entities) {
            ...
            this.updateEntityAI(ent, px, py, dt);
        }
    }

    updateEntityAI(ent, px, py, dt) {
        const dx = px - ent.x;
        const dy = py - ent.y;
    ...
    render() {
        ...
        const faces = [];
        const range = 8;
        this.collectVisibleWalls(x, y, cosA, sinA, faces, range);
        this.collectVisibleEntities(x, y, cosA, sinA, faces, range);
        ...
        for (const f of faces) {
            if (f.type === 'sprite') {
                this.drawSprite(ctx, f);
            } else if (f.type === 'wall-decal') {
                this.drawWallDecal(ctx, f);
            } else if (f.type === 'vector-wall-decal') {
                this.drawVectorWallDecal(ctx, f);
            } else if (f.type === 'vector-sprite') {
                this.drawVectorSprite(ctx, f);
            } else {
                this.drawWall(ctx, f);
            }
        }
    ...
    collectVisibleWalls(x, y, cosA, sinA, faces, range) {
    ...
    collectVisibleEntities(x, y, cosA, sinA, faces, range) {
        this.entities.forEach(ent => {
            ...
            if (ent.type === 'decal') {
                ...
                if (def.sprite) {
                    if (s1.sx > s2.sx) {
                        faces.push({
                            type: 'wall-decal', sprite: def.sprite,
                            x1: s2.sx, yT1: s2.yT, yB1: s2.yB, x2: s1.sx, yT2: s1.yT, yB2: s1.yB,
                            dist: ((c1.rz + c2.rz) / 2) - 0.001, u1: u2, u2: u1, z1: c2.rz, z2: c1.rz,
                            vScale: def.scale || 1.0, vOffset: def.yOffset || 0.0
                        });
                    } else {
                        faces.push({
                            type: 'wall-decal', sprite: def.sprite,
                            x1: s1.sx, yT1: s1.yT, yB1: s1.yB, x2: s2.sx, yT2: s2.yT, yB2: s2.yB,
                            dist: ((c1.rz + c2.rz) / 2) - 0.001, u1: u1, u2: u2, z1: c1.rz, z2: c2.rz,
                            vScale: def.scale || 1.0, vOffset: def.yOffset || 0.0
                        });
                    }
                } else if (hasVector) {
                    faces.push({
                        type: 'vector-wall-decal', monsterType: typeKey,
                        x1: s1.sx, yT1: s1.yT, yB1: s1.yB, x2: s2.sx, yT2: s2.yT, yB2: s2.yB,
                        dist: ((c1.rz + c2.rz) / 2) - 0.001, z1: c1.rz, z2: c2.rz,
                        vScale: def.scale || 1.0, vOffset: def.yOffset || 0.0
                    });
                }
                return;
            ...
            const bobFrame = Math.floor((ent.bobTime || 0) * 0.75) % 2;
            ...
            if (def.sprite) {
                ...
                if (def.onFloor) sy += f * (0.5 - scale / 2 + yOffset);
                else sy += ((def.anchorY ?? 0) + yOffset) * f;
                
                faces.push({
                    type: 'sprite', sprite: def.sprite,
                    sx, sy, size: f * scale, dist: rz - (lunge * 0.01), hitTimer: ent.hitTimer || 0
                });
            } else if (hasVector) {
                const rx = -dx * sinA + dy * cosA;
                const f = this.focalLength / distWithLunge;
                ...
                faces.push({
                    type: 'vector-sprite', monsterType: typeKey,
                    sx, sy, size: f * scale, dist: distWithLunge, hitTimer: ent.hitTimer || 0
                });
            }
        });
    }
    ...
    drawWallDecal(ctx, f) {
        ...
        const img = f.sprite;
        const isReady = img && ((img instanceof HTMLCanvasElement) || (img.complete && img.naturalWidth > 0));
        if (!isReady || screenWidth <= 0.5) return;
